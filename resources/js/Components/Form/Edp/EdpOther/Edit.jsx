...
const EditOtherEdp = ({ Edp, EdpId, onCloseModal }) => {
  ...
  useEffect(() => {
    if (Edp && Object.keys(Edp).length > 0) {
      ...
      setSelectedDates({
        tanggal_mulai: Edp.tanggal_dimulai ? new Date(Edp.tanggal_dimulai) : null,
        tanggal_akhir: Edp.tanggal_selesai ? new Date(Edp.tanggal_selesai) : null,
      });
    }
  }, [Edp]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    put(route("petugas.data.edp-other.update", EdpId), {
      onSuccess: () => {
