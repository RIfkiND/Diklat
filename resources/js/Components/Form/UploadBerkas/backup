...
const UploadForm = ( {petugas,edpother,edpsiswa }) => {
  ...
  const formPetugas = [
    {
      ...
      name: "petugas_1",
      type: "select",
      options: petugas.map((p) => ({ label: p.name, value: p.name })), // Fixed: Removed the unnecessary array wrapper
    },
    {
      title: "Petugas 2",
      name: "petugas_2",
      type: "select",
      options: petugas.map((p) => ({ label: p.name, value: p.name }))
    },
  ];
        ...
        {formPetugas.map((field, index) => (
          <div className="basis-1/2" key={index}>
            ...
            <Select
              ...
              value={
                field.name === "petugas_1"
                  ? field.options.find((option) => option.value === petugas1)
                  : field.name === "petugas_2"
                    ? field.options.find((option) => option.value === petugas2)
                    : null
              }
              onChange={(selectedOption) => {
                if (field.name === "petugas_1") {
                  setPetugas1(selectedOption?.value || null);
                } else if (field.name === "petugas_2") {
                  setPetugas2(selectedOption?.value || null);
                }
              }}
              isSearchable={true} // Pastikan ini diset ke true untuk fitur pencarian
